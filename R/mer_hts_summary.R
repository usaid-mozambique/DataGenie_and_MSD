...
hts_tst <- df %>%
  filter(indicator == "HTS_TST", disag == "Total Numerator") %>%
  ...
  rename(hts_tst_Q1 = Q1, hts_tst_Q2 = Q2, hts_tst_Q3 = Q3, hts_tst_Q4 = Q4, hts_tst_target = target) %>%
  ungroup()

hts_tst_pos <- df %>%
  filter(indicator == "HTS_TST_POS", disag == "Total Numerator") %>%
  select(site_id, partner, agency, indicator, disag, SNU1, PSNU, site, Fiscal_Year, Q1, Q2, Q3, Q4, target) %>%
  group_by(agency) %>%
  summarize(Q1 = sum(Q1, na.rm = TRUE), Q2 = sum(Q2, na.rm = TRUE), Q3 = sum(Q3, na.rm = TRUE), Q4 = sum(Q4, na.rm = TRUE), target = sum(target, na.rm = TRUE)) %>%
  mutate(per_hts_pos_ytd_vs_target = (Q1 + Q2 + Q3 + Q4)/target) %>%
  rename(hts_tst_pos_Q1 = Q1, hts_tst_pos_Q2 = Q2, hts_tst_pos_Q3 = Q3, hts_tst_pos_Q4 = Q4, hts_tst_pos_target = target) %>%
  ungroup()
...
hts_tst_agency <- hts_tst %>%
  mutate(yield_Q1 = hts_tst_pos_Q1 / hts_tst_Q1, yield_Q2 = hts_tst_pos_Q2 / hts_tst_Q2, yield_Q3 = hts_tst_pos_Q3 / hts_tst_Q3, yield_Q4 = hts_tst_pos_Q4 / hts_tst_Q4) %>%
